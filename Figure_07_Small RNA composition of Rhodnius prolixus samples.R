...
# Building THE PLOT
################################################################################
Gut_pie_data <- data.frame(GUT_to_plot$rna_type)
...
Gut_pie_data <- within(Gut_pie_data, {
  f <- n < Gut_pie_data[Gut_pie_data$rna_type == 'R. prolixus conserved miRNA',]$n # number os known miRNA in this tissue
  rna_type[f] <- 'Other'
})
Gut_pie_data <- Gut_pie_data[,-grep("f",colnames(Gut_pie_data))]
Gut_pie_data <- aggregate(n ~ rna_type, data=Gut_pie_data, FUN=sum)
Gut_pie_data <- Gut_pie_data %>%
  mutate(per=`n`/sum(`n`)) %>%
  mutate_at(vars(per), funs(round(., 2)))
Gut_pie_data$percentage <- scales::percent(Gut_pie_data$per)
Gut_pie_data$label <- paste(Gut_pie_data$n, " (", Gut_pie_data$percentage, ")", sep = "")
Gut_pie_data$tissue <- "Gut"


Hemolymph_pie_data <- data.frame(HEM_to_plot$rna_type)
colnames(Hemolymph_pie_data) <- c("rna_type")
Hemolymph_pie_data <- Hemolymph_pie_data %>%
  group_by(rna_type) %>%
  count() %>%
  ungroup() 
Hemolymph_pie_data <- within(Hemolymph_pie_data, {
  f <- n < Hemolymph_pie_data[Hemolymph_pie_data$rna_type == 'R. prolixus conserved miRNA',]$n # number os known miRNA in this tissue
  rna_type[f] <- 'Other'
})
Hemolymph_pie_data <- Hemolymph_pie_data[,-grep("f",colnames(Hemolymph_pie_data))]
Hemolymph_pie_data <- aggregate(n ~ rna_type, data=Hemolymph_pie_data, FUN=sum)
Hemolymph_pie_data <- Hemolymph_pie_data %>%
  mutate(per=`n`/sum(`n`)) %>%
  mutate_at(vars(per), funs(round(., 2)))
Hemolymph_pie_data$percentage <- scales::percent(Hemolymph_pie_data$per)
Hemolymph_pie_data$label <- paste(Hemolymph_pie_data$n, " (", Hemolymph_pie_data$percentage, ")", sep = "")
Hemolymph_pie_data$tissue <- "Hemolymph"



Salivary_gland_pie_data <- data.frame(GLA_to_plot$rna_type)
colnames(Salivary_gland_pie_data) <- c("rna_type")
Salivary_gland_pie_data <- Salivary_gland_pie_data %>%
  group_by(rna_type) %>%
  count() %>%
  ungroup() 
Salivary_gland_pie_data <- within(Salivary_gland_pie_data, {
  f <- n < Salivary_gland_pie_data[Salivary_gland_pie_data$rna_type == 'R. prolixus conserved miRNA',]$n # number os known miRNA in this tissue
  rna_type[f] <- 'Other'
})
Salivary_gland_pie_data <- Salivary_gland_pie_data[,-grep("f",colnames(Salivary_gland_pie_data))]
Salivary_gland_pie_data <- aggregate(n ~ rna_type, data=Salivary_gland_pie_data, FUN=sum)
Salivary_gland_pie_data <- Salivary_gland_pie_data %>%
  mutate(per=`n`/sum(`n`)) %>%
  mutate_at(vars(per), funs(round(., 2)))
Salivary_gland_pie_data$percentage <- scales::percent(Salivary_gland_pie_data$per)
Salivary_gland_pie_data$label <- paste(Salivary_gland_pie_data$n, " (", Salivary_gland_pie_data$percentage, ")", sep = "")
...
write.table(
  All_pie, 
  file.path(basepath, "csv_files/All_pie.tab"), 
  append = FALSE, 
  sep = "\t", 
  row.names = FALSE, 
  col.names = TRUE)
...
ggsave(file = file.path(basepath, "Plots", "Figure_07_Small RNA composition of Rhodnius prolixus samples..pdf"), plot, width = 360, height = 480, units = "mm")
